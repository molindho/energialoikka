...
{	if (!options) options={};
	var element = jQuery("<div>").addClass('ol-unselectable ol-control ol-bar');
...
ol.control.Bar.prototype.setVisible = function (val) {
	if (val) jQuery(this.element).show();
	else jQuery(this.element).hide();
}
...
{	return (jQuery(this.element).css('display') != 'none');
}
...
{	jQuery(this.element).removeClass('ol-left ol-top ol-bottom ol-right');
	pos=pos.split ('-');
	...
	{	switch (pos[i])
		{	case 'top':
			...
			case 'bottom':
			case 'right':
				jQuery(this.element).addClass ("ol-"+pos[i]);
				break;
			default: break;
		}
	}
